  ...
  const onSubmitHandle = async (event) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        localStorage.setItem("employee_id", response.data.employee._id);
        localStorage.setItem("agent_code", response.data.employee.agent_code);
        localStorage.setItem("totalBalance", response.data.employee.balance);
        localStorage.setItem(
          "accessibleBalance",
          response.data.employee.accessibleBalance
        );

        sendSuccessNotification("Welcome " + response.data.employee.email);
        navigate("/");
      } else {
        props.setIsLoading(false);
